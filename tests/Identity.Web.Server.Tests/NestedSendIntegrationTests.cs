	...
	async Task RegisterCommand_dispatch_runs_EmailExistsHandler_mid_validation_with_no_lifetime_errors()
	{
		...
		builder.Services
			...
			.AddAuthorization(options =>
				options.AddPolicy(AuthNPolicies.Public, policy => policy.RequireAssertion(_ => true)))
		...
		RegisterCommand command = new(new RegisterRequest
		{
			EmailInput = "nested-send@example.com",
