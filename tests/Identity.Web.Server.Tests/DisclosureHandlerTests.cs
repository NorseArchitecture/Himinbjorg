...
public sealed class DisclosureHandlerTests(PostgresIdentityFixture fixture)
{
	...
	async Task Self_disclosure_returns_full_decrypted_wire_strings()
	{
		...
		var user = await fixture.SeedUserAsync("me@example.com", "+15551234567");
	...
	async Task Self_disclosure_with_no_user_id_claim_fails_loudly_naming_the_claim_type()
	{
		...
		var exception = await Should.ThrowAsync<InvalidOperationException>(async () =>
			await handler.Handle(new(new GetMyPersonalDataRequest()), TestContext.Current.CancellationToken));

		exception.Message.ShouldContain(new IdentityOptions().ClaimsIdentity
			.UserIdClaimType); // names the missing claim type, not an opaque Guid.Parse(null) failure
	}
	...
	{
		var (context, _) = await fixture.CreateScopeAsync();
		var user = await fixture.SeedUserAsync("jane@domain.com", "+15551234567");
		GetMaskedPersonalDataHandler handler = new(context);

		var outcome = await handler.Handle(new(new GetMaskedPersonalDataRequest { SubjectId = user.Id }),
	...
	async Task Masked_disclosure_of_an_email_less_subject_answers_empty_string_not_a_fault()
	{
		...
		outcome.TryGetValue(out Success<MaskedPersonalDataResponse> success)
			.ShouldBeTrue(); // not a Fault -- a legal row shape
	...
	async Task Reading_a_shredded_subject_answers_erased_with_the_receipt()
	{
		...
		var shred = await new ErasureService(context, keyStore).ShredAsync(user.Id,
			TestContext.Current.CancellationToken);
		...
		failed.Problem.Receipt
			.ShouldBe(receipt.Value); // spec §8 verify item 11: the typed exception crossed EF materialization intact
	}
	...
	async Task Unknown_subject_answers_not_found_not_erased()
	{
		...
		var outcome = await handler.Handle(new(new GetMaskedPersonalDataRequest { SubjectId = Guid.NewGuid() }),
			TestContext.Current.CancellationToken);
